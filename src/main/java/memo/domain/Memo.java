...
public class Memo {
    ...
    public Memo(UUID id, String content, Instant created, Instant modified) {
        this.id = id;
    ...
    public static Memo create(UUID id, String content) {
        UUID memoId = (id != null) ? id : UUID.randomUUID();

        return new Memo(memoId, content, Instant.now(), Instant.now());
    }

    public void update(String content, Instant modified) {
        this.title = this.extractTitleFromContent(content);
        this.content = content;
        this.modified = modified != null ? modified : Instant.now();
        this.deleted = null;
